import { leagueID as fallbackLeagueID } from "$lib/utils/leagueInfo"
import { round } from "$lib/utils/helperFunctions/universalFunctions"
import { waitForAll } from "$lib/utils/helperFunctions/multiPromise"
import { json, error } from '@sveltejs/kit';

export async function GET({ url }) {
    const targetLeagueID = url.searchParams.get('league') || fallbackLeagueID;

    const [nflStateRes, leagueDataRes, playoffsRes] = await waitForAll(
        fetch(`https://api.sleeper.app/v1/state/nfl`, {compress: true}),
        fetch(`https://api.sleeper.app/v1/league/${targetLeagueID}`, {compress: true}),
        fetch(`https://api.sleeper.app/v1/league/${targetLeagueID}/winners_bracket`, {compress: true}),
    )
    ...
    // FIXED: Safely handle pre-draft leagues with no playoff brackets generated yet
    let playoffLength = 3;
    if (playoffs && Array.isArray(playoffs) && playoffs.length > 0) {
        const lastPlayoff = playoffs.pop();
        if (lastPlayoff && lastPlayoff.r) {
            playoffLength = lastPlayoff.r;
        }
    }
    ...
    for(const res of responses) {
        if(!res.ok) {
            throw error(500, "No luck fetching projections");
        }
